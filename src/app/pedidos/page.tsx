    ...
    <>
      ...
      <main className="min-h-screen bg-black pt-24 pb-16 px-4">
        <div className="max-w-3xl mx-auto">
          <h1 className="text-2xl font-black tracking-tight uppercase mb-8">MIS PEDIDOS</h1>
          ...
          {orders.length === 0 ? (
            ...
            <div className="space-y-4">
              {orders.map((order) => {
                const st = statusLabels[order.status] || { label: order.status, color: "text-white/60" };
                return (
                  <div key={order.id} className="bg-white/5 border border-white/10 p-5">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <p className="text-[10px] text-white/40 tracking-wider">ORDEN #{order.id.slice(-8).toUpperCase()}</p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-sm font-bold mt-0.5">₡{order.total.toLocaleString()}</p>
                      </div>
                    </div>
