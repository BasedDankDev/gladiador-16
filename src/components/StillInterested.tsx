          ...
          {products.slice(0, 4).map((p) => (
            <div key={p.slug} className="group relative">
              <Link href={`/producto/${p.slug}`} className="block">
                ...
                <div className="pt-3">
                  <p className="text-[10px] text-black/40 uppercase tracking-[0.15em]">{p.brand}</p>
                  <h3 className="text-sm font-medium text-black mt-0.5">{p.name}</h3>
                  <p className="text-sm font-bold text-black mt-1">₡{p.price.toLocaleString()},00</p>
                </div>
              </Link>
              ...
              <button
                ...
                className="absolute top-2 right-2 w-7 h-7 bg-white/90 hover:bg-white text-black/60 hover:text-black rounded-full flex items-center justify-center shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"
              >
